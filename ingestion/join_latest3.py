...
def load_latest3_data(data_dir: str) -> pd.DataFrame:
    """
    Loads all latest3 indicator files for all constituencies,
    returning a single long-format DataFrame.
    """
    base_dir = Path(data_dir) / "raw/latest3"
    records = []
    ...
    print(f"Found {len(files)} files in raw/latest3 folder. Scanning and parsing...")
...
if __name__ == "__main__":
    ...
    df = load_latest3_data(base_data_dir)
    if not df.empty:
