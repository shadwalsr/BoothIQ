...
def run_completeness_check(df_master: pd.DataFrame, data_dir: str):
    ...
    has_latest2 = df_master['has_latest2_match'].sum() if 'has_latest2_match' in df_master.columns else 0
    has_latest3 = df_master['has_latest3_match'].sum() if 'has_latest3_match' in df_master.columns else 0
    ...
    metrics = {
        ...
        'latest2_match_pct': (has_latest2 / total) * 100,
        'latest3_match_pct': (has_latest3 / total) * 100,
