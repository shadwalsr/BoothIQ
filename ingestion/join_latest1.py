...
def load_latest1_data(data_dir: str) -> pd.DataFrame:
    """
    Loads all latest1 indicator files for all constituencies,
    returning a single long-format DataFrame.
    """
    base_dir = Path(data_dir) / "raw/latest1"
    records = []
    ...
    print(f"Found {len(files)} files in raw/latest1 folder. Scanning and parsing...")
...
if __name__ == "__main__":
    ...
    df = load_latest1_data(base_data_dir)
    if not df.empty:
